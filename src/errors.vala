...
public errordomain ACC.AError {
    ...
    public static AError from_error (Error e) {
        if (e is IOError) {
            var regex = /^GDBus\.Error:org\.altlinux\.APM\.Error\.[A-Za-z]+:\s+(.*)$/;  // vala-lint=space-before-paren
            MatchInfo mi;
            if (regex.match (e.message, 0, out mi)) {
                return new AError.APM (mi.fetch (1));
            }
